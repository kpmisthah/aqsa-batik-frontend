...
export default function ProductCard({ product, isWholesalePage = false }: ProductCardProps) {
    ...
    return (
        <div className="group flex flex-col gap-6 h-full">
            {/* Image Container */}
            <div className="relative aspect-square rounded-[24px] overflow-hidden shadow-xl border border-primary/5 transition-all duration-700 group-hover:-translate-y-2">
                <Image
                    src={imageSrc}
                    alt={product.name}
                    layout="fill"
                ...
                {product.isBestSeller && (
                    <div className="absolute top-6 left-6 bg-secondary text-white text-[10px] font-black px-4 py-2 rounded-full uppercase tracking-widest shadow-lg z-20">
                        Best Seller
                    </div>
                )}
            </div>
            ...
            <div className="flex flex-col gap-4 px-2 text-center items-center flex-grow">
                <div className="flex flex-col gap-1 items-center">
                    <span className="text-[9px] md:text-[11px] font-bold text-secondary/60 uppercase tracking-[0.3em]">
                        {product.subCategory}
                    </span>
                    <h4 className="font-heading text-[15px] md:text-2xl font-bold text-primary leading-tight">
                        {product.name}
                    </h4>
                </div>
                ...
                <div className="flex flex-col gap-2 mt-auto w-full px-1">
                    <Link
                        href={productHref}
                        className="w-full py-2.5 md:py-4 border border-primary/20 rounded-lg md:rounded-xl font-bold text-[9px] md:text-[13px] uppercase tracking-[0.1em] md:tracking-widest text-primary hover:bg-primary hover:text-white transition-all duration-300 flex justify-center items-center"
                    >
                        View Details
                    </Link>
                    <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="w-full py-2.5 md:py-4 bg-primary text-white rounded-lg md:rounded-xl font-bold text-[9px] md:text-[13px] uppercase tracking-[0.1em] md:tracking-widest flex items-center justify-center gap-1.5 hover:bg-secondary transition-all duration-300 shadow-md"
                    >
