                ...
                <div className="flex flex-col gap-10 md:gap-14 lg:pl-10 w-full lg:w-1/2 order-3 lg:order-2 justify-center">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-4 md:gap-6">
                        {items.map((item, i) => {
                            ...
                            return (
                                <div key={i} className={`flex ${isComplex ? 'flex-col items-center text-center' : 'flex-col sm:flex-row items-center text-center sm:text-left'} gap-3 sm:gap-4 p-5 md:p-6 bg-white/90 backdrop-blur-md rounded-[16px] border border-primary/10 hover:border-primary/20 hover:shadow-xl transition-all duration-300 group`}>
                                    <div className={`w-10 h-10 sm:w-12 sm:h-12 shrink-0 bg-brand/10 rounded-[12px] flex items-center justify-center text-brand group-hover:bg-brand group-hover:text-white transition-all duration-300`}>
                                    ...
                                    {isComplex ? (
                                        <div className="flex flex-col gap-1.5 w-full items-center">
                                            <h3 className="text-[16px] md:text-[18px] text-primary font-medium leading-tight text-center">{titleText}</h3>
                                            <p className="text-[13px] md:text-[14px] text-muted font-normal leading-relaxed text-center">{descText}</p>
                                        </div>
                                    ) : (
                                        <h3 className="text-h4 text-primary w-full" dangerouslySetInnerHTML={{ __html: titleText }} />
                                    )}
                                </div>
                            )
                        })}
